...
def test_create_card(client, auth_headers):
    ...
    response = client.post("/api/cards", json={"list_id": list_id, "title": "My Task"}, headers=auth_headers)
...
def test_get_card_detail(client, auth_headers):
    board = client.post("/api/boards", json={"name": "Board"}, headers=auth_headers).json()
    list_id = board["lists"][0]["id"]
...
def test_update_card(client, auth_headers):
    board = client.post("/api/boards", json={"name": "Board"}, headers=auth_headers).json()
    list_id = board["lists"][0]["id"]
    card = client.post("/api/cards", json={"list_id": list_id, "title": "Old"}, headers=auth_headers).json()
...
def test_delete_card(client, auth_headers):
    board = client.post("/api/boards", json={"name": "Board"}, headers=auth_headers).json()
    list_id = board["lists"][0]["id"]
    card = client.post("/api/cards", json={"list_id": list_id, "title": "Delete Me"}, headers=auth_headers).json()
...
def test_move_card_within_list(client, auth_headers):
    board = client.post("/api/boards", json={"name": "Board"}, headers=auth_headers).json()
    list_id = board["lists"][0]["id"]
    card1 = client.post("/api/cards", json={"list_id": list_id, "title": "First"}, headers=auth_headers).json()
    card2 = client.post("/api/cards", json={"list_id": list_id, "title": "Second"}, headers=auth_headers).json()
...
def test_move_card_between_lists(client, auth_headers):
    board = client.post("/api/boards", json={"name": "Board"}, headers=auth_headers).json()
    todo_id = board["lists"][0]["id"]
    done_id = board["lists"][2]["id"]
    card = client.post("/api/cards", json={"list_id": todo_id, "title": "Move Me"}, headers=auth_headers).json()
...
def test_archive_card(client, auth_headers):
    board = client.post("/api/boards", json={"name": "Board"}, headers=auth_headers).json()
    list_id = board["lists"][0]["id"]
    card = client.post("/api/cards", json={"list_id": list_id, "title": "Archive Me"}, headers=auth_headers).json()
    response = client.put(f"/api/cards/{card['id']}", json={"archived": True}, headers=auth_headers)
    assert response.status_code == 200
