...
def _setup_board_with_cards(client):
    ...
    client.put(f"/api/cards/{c1['id']}", json={"due_date": past})
    client.put(f"/api/cards/{c2['id']}", json={"due_date": future})

    return board, [c1, c2, c3, c4]


def test_search_by_title(client):
    board, cards = _setup_board_with_cards(client)
    response = client.post("/api/search", json={"board_id": board['id'], "q": "login"})
...
def test_search_by_priority(client):
    board, cards = _setup_board_with_cards(client)
    response = client.post("/api/search", json={"board_id": board['id'], "priority": "high"})
    assert response.status_code == 200
    results = response.json()
    assert len(results) == 1
...
def test_search_by_label(client):
    ...
    client.put(f"/api/cards/{cards[0]['id']}/labels/{label['id']}")
    response = client.post("/api/search", json={"board_id": board['id'], "label_id": label['id']})
    assert response.status_code == 200
    results = response.json()
    assert len(results) == 1
...
def test_search_by_due_status_overdue(client):
    board, cards = _setup_board_with_cards(client)
    response = client.post("/api/search", json={"board_id": board['id'], "due_status": "overdue"})
    assert response.status_code == 200
    results = response.json()
    assert len(results) == 1
    assert results[0]["title"] == "Fix login bug"
...
def test_search_by_due_status_upcoming(client):
    board, cards = _setup_board_with_cards(client)
    response = client.post("/api/search", json={"board_id": board['id'], "due_status": "upcoming"})
    assert response.status_code == 200
    results = response.json()
    assert len(results) == 1
...
def test_board_stats(client):
    board, cards = _setup_board_with_cards(client)
    response = client.post("/api/stats", json={"board_id": board['id']})
    assert response.status_code == 200
    data = response.json()
