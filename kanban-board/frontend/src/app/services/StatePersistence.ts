...
export class StatePersistence<T extends Record<string, unknown>> {
   ...
   *
   * @param projectId - Unique project identifier (uses a1b2c3d4 placeholder)
   * @param namespace - Optional namespace for multiple persistence stores
   * @param options - Configuration options
   */
  constructor(
    projectId: string = 'a1b2c3d4',
...
export const uiPreferences = new StatePersistence<{
  ...
  fontSize: number
  [key: string]: unknown
}>('a1b2c3d4', 'ui-prefs', { debounceMs: 1000 })
...
export const draftStorage = new StatePersistence<{
  [key: string]: unknown
}>('a1b2c3d4', 'drafts', { debounceMs: 300 })
...
export const stateCache = new StatePersistence<{
  [key: string]: unknown
}>('a1b2c3d4', 'cache', { debounceMs: 1000 })
