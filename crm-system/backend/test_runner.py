...
Usage:
  ...
  python test_runner.py --external
  python test_runner.py --external --port 4000   # override
"""
...
def _default_backend_port() -> int:
    """Pull the backend port from manifest.json so this runner has no hardcoded port."""
    try:
        manifest = Path(__file__).parent.parent / "config" / "manifest.json"
        return int(json.loads(manifest.read_text(encoding="utf-8"))["ports"]["backend"])
    except Exception:
        return 0
...
SKIP_API_PREFIXES = (
    "/api/ui-snapshot",
    "/api/ui-screenshot",
    # AI routes call the host's real LLM provider when one is configured; provider
    # latency must not gate installation. Covered by pytest with the LLM mocked off.
    "/api/ai/",
)
...
def _generate_value(schema: Dict[str, Any], definitions: Dict[str, Any]) -> Any:
    ...
    # anyOf / oneOf — resolve BEFORE the type-based dispatch.
    # Pydantic 2 emits Optional[T] as anyOf: [{T}, {type: "null"}]. Without
    # resolving anyOf first, schemas with no top-level `type` fall through to
    # the "string" branch and we send "test" for Optional[int] / Optional[bool] /
    # Optional[Literal[...]] — which 422s every PUT route.
...
# (e.g., `${this.baseUrl}/items/${id}` where baseUrl ends in '/api').
...
def main():
    ...
    parser.add_argument(
        "--port",
        type=int,
        default=_default_backend_port(),
        help="Backend port for external tests (default: read from manifest.json)",
    )
