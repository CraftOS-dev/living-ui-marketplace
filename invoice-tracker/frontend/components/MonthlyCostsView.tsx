...
export const MonthlyCostsView: React.FC<MonthlyCostsViewProps> = ({
  ...
  const allMonthsData = useMemo<MonthData[]>(() => {
    ...
    return sortedKeys.map((key) => {
      ...
      return {
        ...
        shortLabel: `${SHORT_MONTH_NAMES[mIdx]} ${y}`,
  ...
  const activeMonth = useMemo(() => {
    return allMonthsData.find((m) => m.key === selectedMonthKey) || allMonthsData[0]
  ...
  const currentMonthData = useMemo(() => {
    ...
    return allMonthsData.find((m) => m.key === nowKey) || allMonthsData[0]
  }, [allMonthsData])

  const handleDownloadReport = () => {
    const targetMonth = activeMonth || currentMonthData
