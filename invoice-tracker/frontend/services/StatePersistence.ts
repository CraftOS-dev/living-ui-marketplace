...
export class StatePersistence<T extends Record<string, unknown>> {
   ...
   *
   * @param projectId - Unique project identifier (uses invoice-tracker placeholder)
   * @param namespace - Optional namespace for multiple persistence stores
   * @param options - Configuration options
   */
  constructor(
    projectId: string = 'invoice-tracker',
...
export const uiPreferences = new StatePersistence<{
  ...
  fontSize: number
  [key: string]: unknown
}>('invoice-tracker', 'ui-prefs', { debounceMs: 1000 })
...
export const draftStorage = new StatePersistence<{
  [key: string]: unknown
}>('invoice-tracker', 'drafts', { debounceMs: 300 })
...
export const stateCache = new StatePersistence<{
  [key: string]: unknown
}>('invoice-tracker', 'cache', { debounceMs: 1000 })
