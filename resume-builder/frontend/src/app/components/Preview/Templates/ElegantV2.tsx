...
export const ElegantV2: React.FC = () => {
    ...
    switch (key) {
      ...
      case 'experience':
        return sectionVisibility.experience && experiences.length > 0 ? (
          <div key="experience">
            <h3 style={{ fontSize: '0.9rem', fontWeight: 900, textTransform: 'uppercase', letterSpacing: '0.05em', color: accentColor, marginBottom: '14px' }}>
              Experience
            </h3>

            <div style={{ position: 'relative', paddingLeft: '20px', borderLeft: `2px solid ${accentColor}`, marginLeft: '6px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
              {experiences.map(exp => (
  ...
  const leftKeys = ['summary', 'experience', 'projects', 'certifications', 'publications'];
  const rightKeys = ['mostProudOf', 'education', 'skills', 'myTime', 'philosophy'];

  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 260px', minHeight: '1123px', height: '100%', backgroundColor: '#ffffff' }}>
      {/* Left Main Area (White Background + Visual Timeline) */}
