...
import { Phone, Mail, MapPin, Linkedin, Github, Code, Globe } from 'lucide-react';

const TECH_BADGE_COLORS = ['#3b82f6', '#8b5cf6', '#10b981', '#f59e0b', '#ec4899'];

export const TimelineTech: React.FC = () => {
          ...
          <div key="experience">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              {experiences.map((exp, idx) => {
                const badgeBg = TECH_BADGE_COLORS[idx % TECH_BADGE_COLORS.length];
                return (
                  <div key={exp.id}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div style={{ width: '26px', height: '26px', borderRadius: '6px', backgroundColor: badgeBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <Code size={14} color="#ffffff" />
                        </div>
                        <span style={{ fontWeight: 800, fontSize: '0.925rem', color: '#0f172a' }}>{exp.position}</span>
                      </div>
                      <span style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600 }}>{exp.startDate} – {exp.endDate}</span>
                    </div>
                    <div style={{ fontSize: '0.825rem', fontWeight: 700, color: badgeBg, marginLeft: '36px', marginBottom: '4px' }}>
                      {exp.company} {exp.location ? `• ${exp.location}` : ''}
                    </div>
                    <div 
                      style={{ fontSize: '0.825rem', color: '#334155', marginLeft: '36px', lineHeight: 'var(--resume-line-height, 1.55)', whiteSpace: 'pre-line' }}
                      contentEditable 
                      suppressContentEditableWarning 
          ...
          <div key="education">
            ...
            {educations.map(edu => (
              <div key={edu.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.825rem', marginBottom: '6px' }}>
                <div>
                  <strong style={{ color: '#0f172a' }}>{edu.degree}</strong> • <span style={{ color: accentColor, fontWeight: 600 }}>{edu.institution}</span>
                </div>
          ...
          <div key="mostProudOf">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
              {mostProudOf.map(item => (
                <div key={item.id} style={{ padding: '10px 14px', borderRadius: '6px', backgroundColor: '#f8fafc', border: `1px solid #cbd5e1`, borderLeft: `4px solid ${accentColor}` }}>
        ...
        return sectionVisibility.projects && projects && projects.length > 0 ? (
          <div key="projects">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {projects.map(proj => (
                <div key={proj.id} style={{ padding: '10px 14px', borderRadius: '6px', backgroundColor: '#f8fafc', border: `1px solid #cbd5e1`, borderLeft: `4px solid ${accentColor}` }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {certifications.map(cert => (
                <div key={cert.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.825rem' }}>
                  <div>
                    <strong style={{ color: '#0f172a' }}>{cert.url ? <a href={formatHref(cert.url, 'url')} target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'underline' }}>{cert.name}</a> : cert.name}</strong> • <span style={{ color: accentColor, fontWeight: 600 }}>{cert.issuer}</span>
                  </div>
      ...
      <div style={{ borderBottom: `2px solid ${accentColor}`, paddingBottom: '16px', marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            ...
            <h2 
              style={{ fontSize: '1.05rem', fontWeight: 700, color: '#334155', marginTop: '4px' }}
              contentEditable 
              suppressContentEditableWarning 
