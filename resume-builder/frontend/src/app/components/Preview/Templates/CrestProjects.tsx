...
export const CrestProjects: React.FC = () => {
  const { resume, updateInlineField } = useResume();
  const { personal, experiences, educations, skills, projects, certifications, publications = [], sectionVisibility, sectionOrder, style } = resume;
        ...
        return sectionVisibility.projects && projects && projects.length > 0 ? (
          <div key="projects">
            <h3 style={{ fontFamily: 'inherit', fontSize: '0.95rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.1em', color: '#0f172a', borderBottom: '1px solid #cbd5e1', paddingBottom: '4px', marginBottom: '12px' }}>
              Projects & Strategic Engagements
            </h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {projects.map(proj => (
                <div key={proj.id} style={{ fontFamily: 'inherit' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
                    <span style={{ fontFamily: 'inherit', fontWeight: 700, fontSize: '0.95rem', color: '#0f172a' }}>
                      {proj.link ? (
                    ...
                    {proj.technologies && <span style={{ fontSize: '0.775rem', color: '#64748b' }}>{proj.technologies}</span>}
                  </div>
                  <div style={{ fontSize: '0.825rem', color: '#334155', marginTop: '2px', lineHeight: 'var(--resume-line-height, 1.55)', fontFamily: 'inherit' }}>
                    {proj.description}
                  </div>
                  {proj.link && (
        ...
        return sectionVisibility.certifications && certifications && certifications.length > 0 ? (
          <div key="certifications">
            ...
            {certifications.map(cert => (
              <div key={cert.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.825rem', marginBottom: '6px', fontFamily: 'inherit' }}>
                <div>
                  <strong style={{ fontFamily: 'inherit', color: '#0f172a' }}>{cert.url ? <a href={formatHref(cert.url, 'url')} target="_blank" rel="noopener noreferrer" style={{ color: 'inherit', textDecoration: 'underline' }}>{cert.name}</a> : cert.name}</strong>, {cert.issuer}
                </div>
