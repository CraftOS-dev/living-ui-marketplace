...
# endpoint is working and correctly refusing based on live state (cooldowns,
# state conflicts), not a code defect. Applies to POST/PUT/PATCH/DELETE only.
BUSINESS_REJECTION_CODES = (400, 409, 422, 429)
_BUSINESS_SLUG_RE = re.compile(r"^[a-z][a-z0-9_]{1,48}$")


def _is_business_rejection(status_code: int, response_body: Any) -> Optional[str]:
    """
    Return the error slug if a 4xx response represents a business-logic
    rejection (e.g. {"detail": "clean_cooldown"}) rather than a code defect.
    Otherwise return None.
    """
    if status_code not in BUSINESS_REJECTION_CODES:
        return None
    if not isinstance(response_body, dict):
        return None
    for key in ("detail", "error", "status", "code"):
        val = response_body.get(key)
        if isinstance(val, str) and _BUSINESS_SLUG_RE.match(val):
            return val
    return None
...
def run_external_tests(port: int) -> Dict[str, Any]:
    ...
    for route in api_routes:
        ...
        if test_result["status"] == "fail":
            ...
            if note:
                logger.info(f"[PASS] {method} {path} — {test_result.get('status_code')} ({note})")
            else:
...
def _test_endpoint(
    ...
    try:
        ...
        if status_code >= 400:
            slug = _is_business_rejection(status_code, test_result.get("response_body"))
            if slug and method in ("POST", "PUT", "PATCH", "DELETE"):
                test_result["note"] = f"business_rejection: {slug}"
            else:
        ...
        slug = _is_business_rejection(e.code, resp_body)
        if slug and method in ("POST", "PUT", "PATCH", "DELETE"):
            test_result["note"] = f"business_rejection: {slug}"
        else:
            test_result["status"] = "fail"
