...
8. **Sparse tabs** — any tab/section that renders with only a line or two of content (e.g. an evolution chain shown as bare clickable name buttons) is a MINOR finding at least; note whether it should be folded into an adjacent tab or given more depth.
9. **Filter completeness** — for any filterable list, check SPEC §3's full field/tag/flag set against the filters actually offered; a filter surface using only the 2–4 most obvious fields when other plausible facets exist (categorical tags, boolean flags, derived groupings) is a MINOR finding.
10. **Secondary-feature depth** — for any feature beyond SPEC's core Musts (bonus/"nice to have" additions), confirm it's built out to the same completeness bar as a Must (e.g. a team/collection builder needs the sub-attributes a real one would have, not just bare slots) — a shallow bolt-on is a MINOR finding.
11. **Sortable table columns** — for any data table/grid backed by more than ~2 sortable-looking numeric or categorical columns (e.g. a moves table with name/type/power), confirm at least the obviously-sortable columns are actually clickable-to-sort; if none are, that's a MINOR finding.
