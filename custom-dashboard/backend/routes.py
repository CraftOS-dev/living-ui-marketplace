...
from models import (
    ...
    WidgetConfig, WeatherCache, CalendarEvent, Task, Note, Reminder, DailyBriefing, Contact,
)
...
WIDGET_IDS = ["clock", "weather", "calendar", "todos", "notes", "reminders", "briefing",
              "calculator", "contacts", "converter", "timer", "qrcode"]
...
class LenientUpdate(BaseModel):
    ...
    def _coerce(cls, values: Any) -> Any:
        if not isinstance(values, dict):
            return values
        for key in ('enabled', 'completed', 'pinned', 'flagged', 'favorite'):
...
class ContactUpdate(LenientUpdate):
...
def set_weather_city(update: CityUpdate, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        ...
        geo_r = httpx.get(geo_url, timeout=4)
...
def list_contacts(db: Session = Depends(get_db)) -> List[Dict[str, Any]]:
    return [c.to_dict() for c in db.query(Contact).order_by(Contact.name.asc()).all()]


@router.post("/contacts")
def create_contact(data: ContactCreate, db: Session = Depends(get_db)) -> Dict[str, Any]:
    contact = Contact(name=data.name, email=data.email, phone=data.phone, notes=data.notes or "")
    db.add(contact)
    db.commit()
    db.refresh(contact)
...
async def generate_briefing(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    # Only pull data from widgets the user currently has enabled
    enabled = {c.widget_id: c.enabled for c in _ensure_widget_configs(db)}

    # Gather data
    pending_tasks = (
        db.query(Task).filter(Task.completed == False).count()
        if enabled.get("todos", True) else 0
    )
    upcoming_reminders = (
        ...
        if enabled.get("reminders", True) else []
    )
    upcoming_events = (
        db.query(CalendarEvent).filter(
            CalendarEvent.event_date >= today
        ).order_by(CalendarEvent.event_date, CalendarEvent.start_time).limit(3).all()
        if enabled.get("calendar", True) else []
    )
    weather = db.query(WeatherCache).first() if enabled.get("weather", True) else None
    ...
    prompt_lines = [
        "Generate a brief, friendly daily briefing for the user.",
        f"Date: {today}",
    ]
    if enabled.get("weather", True):
        prompt_lines.append(f"Weather: {weather_summary}")
    if enabled.get("todos", True):
        prompt_lines.append(f"Pending tasks: {pending_tasks}")
    if enabled.get("reminders", True):
        prompt_lines.append(f"Upcoming reminders:\n{reminders_text}")
    if enabled.get("calendar", True):
        prompt_lines.append(f"Upcoming calendar events:\n{events_text}")
    prompt_lines.append(
