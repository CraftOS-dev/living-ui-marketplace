...
def _add_column_if_missing(table: str, column: str, ddl_type: str) -> None:
    ...
    columns on tables that already exist, so schema additions to models
    need an explicit ALTER TABLE guard here.
    """
    with engine.connect() as conn:
        existing = {row[1] for row in conn.exec_driver_sql(f"PRAGMA table_info({table})")}
        if column not in existing:
            conn.exec_driver_sql(f"ALTER TABLE {table} ADD COLUMN {column} {ddl_type} DEFAULT 0")
            conn.commit()
            logger.info(f"[Database] Added column {table}.{column}")
...
async def init_db():
    ...
    _add_column_if_missing("reminders", "flagged", "BOOLEAN")
    _add_column_if_missing("contacts", "favorite", "BOOLEAN")
