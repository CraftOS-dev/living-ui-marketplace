...
import { Clock, Cloud, CalendarDays, CheckSquare, FileText, Bell, Sparkles, Calculator, Users, Ruler, Timer, QrCode } from 'lucide-react'
...
const WIDGET_INFO: Record<WidgetId, { label: string; description: string; icon: React.ReactNode }> = {
  ...
  calculator:{ label: 'Calculator',    description: 'Evaluate arithmetic expressions with exponents, roots, and logarithms.', icon: <Calculator size={20} /> },
  contacts:  { label: 'Contacts',      description: 'Keep a simple list of names, emails, phone numbers, and notes.',   icon: <Users size={20} /> },
  converter: { label: 'Unit Converter',description: 'Convert values between length, weight, temperature, and volume units.', icon: <Ruler size={20} /> },
  timer:     { label: 'Timer',         description: 'Countdown timer with presets, plus a stopwatch mode.',             icon: <Timer size={20} /> },
  qrcode:    { label: 'QR Code',       description: 'Generate a QR code from any text or URL, with a downloadable PNG.', icon: <QrCode size={20} /> },
}
