...
function PageThumbnail({
  ...
  // V2: the thumbnail URL points at the client-side adapter (sentinel host),
  // which <img> can't reach directly. Fetch it through window.fetch (patched by
  // the adapter), then display the resulting blob; revoke the object URL on
  // cleanup.
  useEffect(() => {
    let cancelled = false
    let objectUrl: string | null = null
    fetch(thumbUrl)
      .then((res) => (res.ok ? res.blob() : Promise.reject(new Error('thumbnail failed'))))
      .then((blob) => {
        if (cancelled) return
