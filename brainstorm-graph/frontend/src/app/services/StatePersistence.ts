...
export class StatePersistence<T extends Record<string, unknown>> {
   ...
   *
   * @param projectId - Unique project identifier (uses brainstorm-graph placeholder)
   * @param namespace - Optional namespace for multiple persistence stores
   * @param options - Configuration options
   */
  constructor(
    projectId: string = 'brainstorm-graph',
...
export const uiPreferences = new StatePersistence<{
  ...
  fontSize: number
  [key: string]: unknown
}>('brainstorm-graph', 'ui-prefs', { debounceMs: 1000 })
...
export const draftStorage = new StatePersistence<{
  [key: string]: unknown
}>('brainstorm-graph', 'drafts', { debounceMs: 300 })
...
export const stateCache = new StatePersistence<{
  [key: string]: unknown
}>('brainstorm-graph', 'cache', { debounceMs: 1000 })
